...
import { getPromoterStats, getSignedInPromoter, listPromoterTickets } from '@/lib/promoters';
...
export default async function PromoterPage() {
  ...
  const [stats, tickets, event] = await Promise.all([
    getPromoterStats(signedIn.id),
    listPromoterTickets(signedIn.id, 50),
    getFeaturedEvent(),
  ]);
    ...
    <Shell>
      ...
      {tickets.length > 0 && (
        <section className="mt-8">
          <h2 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted">Recently issued</h2>
          <ul className="mt-2 divide-y divide-edge/70 overflow-hidden rounded-xl border border-edge bg-paper">
            {tickets.map((t) => (
              <li key={t.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
                <div className="min-w-0">
                  <p className="truncate text-[14px] font-semibold text-ink">{t.holder_name}</p>
