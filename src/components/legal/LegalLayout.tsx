 ...
 */
const PROSE = cn(
  'text-[15px] leading-[1.8] text-haze',
  '[&_p]:mt-4 [&_p:first-child]:mt-0',
  '[&_strong]:font-semibold [&_strong]:text-chalk',
  '[&_h3]:mt-9 [&_h3]:mb-3 [&_h3]:font-display [&_h3]:text-[17px] [&_h3]:font-semibold [&_h3]:tracking-tight [&_h3]:text-chalk',
  '[&_h4]:mt-6 [&_h4]:mb-2 [&_h4]:text-[15px] [&_h4]:font-semibold [&_h4]:text-chalk',
  ...
  '[&_a]:font-medium [&_a]:text-vybe-200 [&_a]:underline [&_a]:decoration-vybe-500/50 [&_a]:underline-offset-4',
  '[&_a:hover]:text-vybe-100 [&_a:hover]:decoration-vybe-400',
  '[&_dl]:mt-4 [&_dl]:space-y-3',
  '[&_dt]:font-semibold [&_dt]:text-chalk',
  '[&_dd]:mt-1 [&_dd]:text-haze',
  '[&_code]:rounded [&_code]:bg-ink [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:font-mono [&_code]:text-[13px] [&_code]:text-pulse-300',
  '[&_address]:not-italic',
);
...
export function LegalCallout({
  ...
  return (
    <div className={cn('card card-lit mt-6 border-vybe-500/25 bg-vybe-950/40 p-5 sm:p-6', className)}>
      <p className="mb-3 font-display text-[15px] font-semibold text-vybe-100">{title}</p>
...
function ShieldIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      className="h-5 w-5 shrink-0 text-vybe-300"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.6"
      strokeLinecap="round"
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="container-hov relative pb-24 pt-16 sm:pt-24 lg:pb-32">
        ...
        <div
          role="note"
          className="card card-lit mt-10 flex items-start gap-4 border-vybe-500/30 bg-vybe-950/50 p-5 sm:p-6"
        >
          <ShieldIcon />
          <div>
            ...
            <p className="mt-2 text-[14px] leading-relaxed text-haze">
              ...
              Telangana, and has <strong className="font-semibold text-chalk">not</strong> been
        ...
        <div className="mt-14 grid gap-12 lg:mt-16 lg:grid-cols-[236px_minmax(0,1fr)] lg:gap-16">
          <nav aria-label="On this page" className="hidden lg:block">
            <div className="sticky top-28 max-h-[calc(100dvh-9rem)] overflow-y-auto pr-1">
              ...
              <div className="relative pl-5">
                <span
                  aria-hidden="true"
                  className="absolute left-0 top-1 h-[calc(100%-0.5rem)] w-px bg-hairline"
                />
                <span
                  aria-hidden="true"
                      ...
                      <li key={section.id}>
                        <a
                          ...
                          className={cn(
                            ...
                            active
                              ? 'bg-vybe-500/10 text-chalk'
                              : 'text-dim hover:bg-white/[0.03] hover:text-haze',
                          )}
                        >
                          <span
                            aria-hidden="true"
                            className={cn(
                              ...
                              active ? 'text-vybe-300' : 'text-dim/70',
                            )}
                          >
          ...
          <div>
            <details className="glass mb-10 rounded-2xl px-5 py-4 lg:hidden">
              <summary className="cursor-pointer list-none font-display text-sm font-semibold text-chalk marker:hidden">
              ...
              <ol className="mt-4 space-y-2 border-t border-hairline pt-4">
                {sections.map((section, index) => (
                  <li key={section.id} className="flex gap-2.5 text-[14px]">
                    ...
                    <a href={`#${section.id}`} className="text-haze hover:text-chalk">
                      {section.title}
                    </a>
                  </li>
                ))}
